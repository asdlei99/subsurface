...
Kirigami.ScrollablePage {
	...
	onRefreshingChanged: {
		if (refreshing) {
			if (PrefCloudStorage.cloud_verification_status === CloudStatus.CS_VERIFIED) {
				...
				manager.appendTextToLog("sync with cloud storage requested, but credentialStatus is " + PrefCloudStorage.cloud_verification_status)
	...
	onBackRequested: {
		if (startPage.visible && diveListView.count > 0 &&
			PrefCloudStorage.cloud_verification_status !== CloudStatus.CS_INCORRECT_USER_PASSWD) {
			PrefCloudStorage.cloud_verification_status = oldStatus
			event.accepted = true;
		}
